...
function SMF(obj::SlideMeanFilter{T}, inp::AbstractVector{T}, out::AbstractVector) where T
    ...
    if obj.need_restart # инициализация на первой точке
        ...
        fill!(buf, inp[1])
        # отмечаем, что инициализация уже не требуеsтся
...
# функция фильтра взвеш скользящего среднего
...
#перевод в вариант
time_samples_v16_s = filter(x -> x>=16*60, time_samples)
